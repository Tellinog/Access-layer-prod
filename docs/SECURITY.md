...
Step 3A implements persistence and validation foundation only; the target protocol remains unimplemented and globally disabled. `OAUTH_P0_ENABLED` defaults to `false`, requires no OAuth secret/key input, and does not register routes even when `true`. `src/oauth/` is not connected to legacy request handling.

Foundation credential tables contain only `secret_hash` plus lifecycle metadata. Repository metadata queries deliberately do not select credential hashes. `oauth_signing_keys` accepts a public RSA JWK with private members constrained out, a public SHA-256 fingerprint, lifecycle metadata and a protected private-key reference; it has no private-key byte/PEM column, and the repository never returns the protected reference. No real credential, private key, token or pilot data is seeded or used in fixtures.
...
OAuth vNext uses the future internal Google callback `/oauth/upstream/google/callback`; it never extends the frozen legacy callback and remains unimplemented and unregistered after Step 3A. Exact downstream client state is kept only in future short-lived reversible protected storage until returned and is never logged. Upstream Google state and nonce may remain hash-only.
