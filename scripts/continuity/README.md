...
They do not modify production or the evidence YAML. They fail closed when required observations are unavailable. The runtime collector treats an empty required secret differently from absence, permits an observed empty `PUBLIC_BASE_PATH`, and screens credential-bearing URLs. Do not change the helpers to print process environments, secret values or reusable verifiers, connection strings, response bodies, private/symmetric JWK members, personal values, SQL row contents, or driver error messages.
