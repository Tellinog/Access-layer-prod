...
V1 legacy implementation complete. The **OAuth P0 runtime + administration candidate** descends from exact commit `bae112404a8432b4d4ce7e3e70d96f2217ac7597`. The checked-in OAuth protocol flag remains default-off; no pilot, production Nancy registration, production key/secret provisioning, production restore or OAuth enablement is evidenced here. The user reports deploying the prior Admin UI change, but this task has not verified the production image or configuration. The full Nancy-shaped Admin-to-OAuth HTTP flow passed on a disposable PostgreSQL 16.15 target at `ce9668f514c9da0c55d33ae4b0da28527595e0fb`; the unchanged Step 4B qualification remains a separate full regression gate. Independent review is still required.

## D-048 Admin grant validation correction, 2026-09-25

- Explicit user approval resolved the frozen-contract gate for the two official `platform_admin` keys `admin:access_requests:read/write`. The shared legacy validator and legacy API schemas now accept those exact keys without widening the general regex or relaxing tool-registration checks. `specs/validation.v1.yml` and D-048 record the exception; the frozen Step 1 baseline remains unchanged and OAuth scope/entitlement grammar remains strict. The user reports deploying the prior UI-only change, but this task has not verified that deployed image or deployed D-048.
- Local API regression verifies that the existing active `access-admin` grant can be updated with all 15 platform-admin keys and `/v1/me` then reports `admin:oauth:read/write`. It also verifies grant creation with the same registered key set. This source change has not been deployed or exercised against the production database by this task; the operator still must update the existing grant after deployment and verify `/admin/oauth`.
...
- The repository's OAuth Admin guard requires the active session grant to carry `platform_admin` plus explicit `admin:oauth:read`/`admin:oauth:write`. The seed and the shown production form include both OAuth keys, so missing catalog registration does not explain the shown attempt. The Admin UI links to OAuth P0 for authorized sessions and displays missing registered permission keys from grant validation errors. D-048 records the subsequently approved validator correction.
...
- Repository lint/build and all 362 Vitest tests pass. Python discovery ran 64 tests with two expected skips, the OAuth contract validator passed all 26 groups, the platform checker passed 27 checks with seven documented warnings, Compose rendered, continuity remained valid but not ready, and `git diff --check` passed. The separate Admin qualifier passed on a fresh disposable PostgreSQL 16.15 database: two canonical scopes, one Nancy BFF client, the Nancy API plus one isolated audience-mismatch resource, exact mappings/allowances, 32 sanitized Admin audit events, secret-free backup/audit assertions and a complete real OAuth HTTP onboarding/negative lifecycle. The unchanged Step 4B qualifier passed on two fresh loopback-only PostgreSQL 16.15 targets, including 8/8 same-refresh races, encrypted replace restore, post-restore continuity and legacy smoke. No production operation or independent approval is claimed.
