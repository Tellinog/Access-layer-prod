...
- **BLOCKER (2026-09-25, Admin OAuth grant):** The user-provided form shows all 15 registered `access-admin` keys selected, including `admin:access_requests:read` and `admin:access_requests:write`. Both keys are in the official `platform_admin` role and seed, but `validatePermissionKey` rejects their underscores, so create/update returns `VALIDATION_ERROR` before checking the tool catalog. A local runtime check confirmed both reject while `admin:oauth:read/write` pass. The frozen legacy permission contract requires explicit approval and a compatibility decision before changing this validator; approval has been requested. No production grant or deployment changed. After an approved correction is deployed, verify the active session grant via `/v1/me` and `/admin/oauth`.
- **QUESTION (2026-09-25, Admin grant details):** The supplied request-completion log omits 400 response bodies, and the browser console excerpt likewise omits `error.details`. The invalid official keys explain the shown all-15-permission form, but inspect any remaining 400 after the compatibility fix. Other documented delegated Admin keys with `_assigned` are outside this targeted correction and need their own compatibility review if grant editing is affected.

## BLOCKER

- 2026-09-24 OAuth administration candidate: the Admin-created Nancy-shaped graph completed real OAuth authorize/callback/code/refresh/introspection/revocation and negative lifecycle tests on fresh disposable PostgreSQL 16.15 at `ce9668f514c9da0c55d33ae4b0da28527595e0fb`. The unchanged full Step 4B qualifier also passed on fresh paired targets at `aefccc263ea6c9495807f3f54c84b52d766b39b2`. Independent review must reproduce the final commit; neither local result is production approval or permission to enable OAuth.
