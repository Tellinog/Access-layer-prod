...
- Revocation and introspection are core services only. Access-token revocation resolves the exact OAuth audience even while its resource is disabled and records the jti until original expiry, so later re-enablement cannot reactivate it; refresh revocation revokes the family/session. Introspection authenticates only resource-owned OAuth credentials, verifies the OAuth JWT locally with exact audience, and checks jti/session/authorization/current entitlement online state; inactive, malformed, wrong-audience, legacy and unknown candidates normalize to `{ "active": false }`.
...
- Final local validation passes: TypeScript lint/build; 278 Vitest tests across 16 files; Python 60 passed with two expected skips; all 24 OAuth validator groups; continuity `VALID_BUT_NOT_READY` with both gates false; non-strict platform 27 passes/seven known warnings; frozen legacy/migration/dependency checks; and `git diff --check`. Migration 005 SHA-256 is `aaffcb469000f62e680b5d391360fdacc4414e4280ba230e90e7fd4eee4dafbe`. Docker/PostgreSQL remained unavailable, so disposable-PG migration and real concurrency evidence remain an explicit release assumption; no production system was contacted.
