...
Status: frozen target contract; Step 3A foundation only; protocol runtime not implemented or enabled
Machine source: `../specs/oauth-p0.v1.yml`
Target OpenAPI: `../schemas/access-layer-oauth-v1.openapi.yaml`

## Boundary and compatibility

P0 is an additive OAuth authorization-server contract. Step 2 created no runtime endpoint, table, migration, dependency, client registration or production configuration. The separately approved Step 3A adds only disabled foundation tables/config/types/repository/validation; it adds no protocol route, transaction table, credential issuance/authentication, token runtime, registration API, seed or production configuration. The frozen `access-layer-legacy-v1` contract remains authoritative for `/v1/*`, Google callback, JWT/JWKS, sessions, refresh tokens, grants, permissions, cookies, tool clients, Admin UI and current consumers. OAuth failure must never silently fall back to legacy credentials, and no consumer is forced to migrate.
...
A scope is granted only when it is registered for the resource, allowed for the client/resource relationship, explicitly mapped to exactly one legacy permission key, present through that exact key in the human's effective active grant, and allowed by central policy. Every resource registration contains one `scope_entitlement_mappings` entry for each declared scope, with no missing, extra or duplicate scope. The mapped key may equal the canonical OAuth scope when the existing permission is identical; otherwise the difference is explicit. Prefix conversion, segment rewriting and inferred aliases are forbidden. The mapped key must be registered for the resource's bound legacy tool. A missing, stale, unknown or ungranted mapping fails closed as `invalid_scope`/deny.

OAuth client identity and resource identity are separate. Every P0 resource registration requires exactly one `legacy_tool` entitlement binding so existing grants and registered permissions remain authoritative. Its `legacy_tool_slug` uses the exact legacy grammar `^[a-z0-9][a-z0-9-]{1,62}[a-z0-9]$`; legacy permission keys use `^[a-z0-9-]+(?::[a-z0-9-]+)+$`. The bridge is entitlement-only: the legacy `tool` is never implicitly the OAuth client or the OAuth resource, and no legacy record changes. Native OAuth entitlement domains are deferred beyond P0.
...
After Step 2 is approved, generic Step 3 implementation may proceed locally/dark with legacy `/v1/*` enabled and OAuth globally disabled by default. A concrete pilot client, resource, scope mapping and resource credential are not prerequisites for generic dark implementation; they are required before any OAuth enablement or production registration.

The first OAuth-capable binary must start with no enabled OAuth client, no legacy key/secret rotation and only backward-compatible additive schema. It must roll back immediately to the previous image without losing legacy or new rows. Production deploy or enablement remains blocked by the Coolify `Changes pending` review, verified backup/restore, destination identity, central registry, named ownership, deployed revision/image identity, secret/key continuity and the later N→N+1 gates. OAuth is enabled only for an explicit reviewed client/resource/scope pilot after those gates. Invalid OAuth credentials are rejected as OAuth; the service never tries legacy credentials as fallback. Any per-client disable restores the pre-change traffic path without deleting OAuth state.
