...
- 2026-08-28 Step 4B gate: Step 4A now covers all 17 current `oauth_*` tables with deterministic repository tests and dependency-safe replace ordering, but a real encrypted full export and replace restore into a second isolated PostgreSQL 16 database is still unexecuted. Before the first non-empty production OAuth registration or pilot, prove restored synthetic OAuth credentials/state/token lifecycle using the same synthetic external continuity secrets/key storage, plus the remaining Step 4B E2E/concurrency/old-binary checks. Never use production as the substitute.
