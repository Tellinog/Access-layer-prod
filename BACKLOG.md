...
- 2026-08-31 Step 4B hardened rerun: exact clean commit `ac3a4c54f4f865d0193f254b9a525f5b06e6b28d` fixed all three reserved SQL aliases and passed real code exchange, claims/TTL/JWKS/introspection, normal refresh and revocation on two disposable PostgreSQL 16 targets. The fail-fast qualification then observed exactly one success for two simultaneous uses of the same refresh credential, but the loser was not the required HTTP 400 `invalid_grant`. Both requests completed before the timeout, but family/session replay-state assertions were not reached. Diagnose and separately approve the real-PG loser-path correction, then rerun Step 4B from a new exact clean commit; do not bypass, enable OAuth, deploy or proceed to Step 5.
...
- 2026-08-29 Step 4B gate: hardened Step 4A covers all 17 current `oauth_*` tables, exports all 24 sections from one read-only repeatable-read snapshot and rejects inconsistent credential/refresh lineage before writes, but a real encrypted full export and replace restore into a second isolated PostgreSQL 16 database is still unexecuted. Before the first non-empty production OAuth registration or pilot, prove restored synthetic OAuth credentials/state/token lifecycle using the same synthetic external continuity secrets/key storage, plus the remaining Step 4B E2E/concurrency/old-binary checks. Never use production as the substitute.
