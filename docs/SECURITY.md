...
Revocation/introspection remain unmounted core methods in Step 3D. Client authentication completes before token lookup. A recognised RFC 7009 hint selects only access-versus-refresh lookup order; a miss always falls through to the other supported type and an unknown hint uses the default order. Access-token revocation resolves only the exact existing OAuth audience, without requiring the resource to remain active and without legacy fallback, then persists the OAuth `jti` until JWT `exp + 60 seconds`. This `expires_at` value is the online revocation-retention deadline, not a change to the JWT's nominal expiry; repeated revocation can only extend it. Refresh revocation revokes its family/session. Resource introspection ignores RFC 7662 hints, verifies only OAuth `RS256`/`at+jwt` keys and exact audience, then checks jti, OAuth session, authorization and current entitlement state. Refresh, unknown, legacy, malformed, expired, wrong-audience and inactive candidates expose only `active:false`.
...
- pending grant email: well-formed company email whose domain is allowed by `GOOGLE_ALLOWED_HD` or configured `MICROSOFT_ALLOWED_EMAIL_DOMAINS`; Testbirds configuration includes `testbirds.com` and `testbirds.de`, while Google login continues to require its own valid `hd`;
...
The 24 backup table reads run on one transaction-bound PostgreSQL connection after `REPEATABLE READ, READ ONLY` is established, preventing a torn export across concurrent authorization or refresh transitions without taking a write lock. Import structure, credential rotation graphs and exact refresh generations are validated before writes; deterministic malformed-backup failures are sanitized caller-validation errors with HTTP 400 semantics and never include row values, hashes, ciphertext or key references.
