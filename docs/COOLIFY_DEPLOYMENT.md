...
This deployment contract is adopted. Live Coolify evidence identifies server `wx513ojqd80kdicevubog7`, project `xdihnb979tvyh9gdk72zfy7y`, environment `rd3mt4dkpqyghxlx9h96sdlo` and resource `u3cyw3y1obp88to9la0w8c75`; destination remains unresolved. It resolves the source declaration to the already-running logical volume `access_layer_postgres_data_v2`, and proves JWT storage `access_layer_jwt_secrets` as a named volume. Do not add explicit physical names or rename either logical/live volume. The visible Coolify `Changes pending` state must be reviewed before any future production deploy. Deployment remains blocked until that review plus backup/restore, registry, ownership and remaining continuity evidence are complete.
