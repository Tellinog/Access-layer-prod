...
RFC 7009 and RFC 7662 token-type hints are advisory rather than token-class filters. Revocation authenticates the client first, uses a recognised hint only to choose lookup order and falls through to the other supported class on a miss; unknown hints use the default lookup order. Introspection ignores the hint and retains access-token-only exact-audience disclosure. Access-token jti revocation retention ends at JWT `exp` plus the frozen 60-second verifier skew, and repeated writes may only extend that deadline. Signing-key `last_signed_at` is stored as the maximum of the existing and observed signing instants so retirement grace cannot be shortened by clock rollback.
