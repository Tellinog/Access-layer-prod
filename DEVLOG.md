...
- On `ce9668f514c9da0c55d33ae4b0da28527595e0fb`, the separate Admin qualifier passed against a fresh disposable PostgreSQL 16.15 target: two scopes, one Nancy BFF client, Nancy API plus one audience-mismatch resource, two allowances, one signing key and 32 sanitized Admin audit events. The Admin-created records drove a real HTTP authorization redirect, callback, code exchange, refresh, active/wrong-audience introspection, revocation, negative registration/grant states and legacy-credential rejection. The unchanged Step 4B qualifier passed on two additional fresh loopback-only PostgreSQL 16.15 targets at `aefccc263ea6c9495807f3f54c84b52d766b39b2`: OAuth HTTP lifecycle, 8/8 same-refresh races, repeatable-read snapshot, encrypted replace restore, post-restore token continuity and legacy smoke. All qualifier containers were removed. Independent review remains open; no production operation was attempted.
