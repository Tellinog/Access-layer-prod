...
Step 3C HTTP emits `oauth.authorization.requested`, `oauth.authorization.allowed`, `oauth.authorization.denied` and `oauth.code.issued`. Step 3D unmounted services add transaction-bound `oauth.code.exchanged`, `oauth.token.refreshed`, `oauth.refresh.replay_detected`, `oauth.token.revoked` and `oauth.token.introspected`. Failed code exchange instead writes sanitized `oauth.code.exchange_denied` in a deliberately separate bounded transaction after the failed main transaction rolls back. If this mandatory denial audit write fails, the operation remains denied and surfaces only `temporarily_unavailable`. Registration/key-management events and Step 3D HTTP request logging remain absent. Automatic Fastify request logs stay silent on `/oauth/authorize` and `/oauth/upstream/google/callback` because their query strings contain state, Google code and PKCE material.
